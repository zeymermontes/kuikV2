-- request_reservation: a booking from the WhatsApp bot does not need a
-- phone number even when the restaurant requires one on the web form — the
-- chat itself is the contact channel, and a LID chat has no number to give.
-- Otherwise the same function as 0064.
CREATE OR REPLACE FUNCTION public.request_reservation(p_tenant uuid, p_branch uuid, p_area uuid, p_name text, p_phone text, p_party integer, p_date date, p_time text, p_note text, p_source text DEFAULT 'form'::text, p_force boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if not v_staff then
    ...
    -- A booking made in a WhatsApp chat is reachable there: the phone the
    -- restaurant requires on the web form is not required of the bot (the
    -- chat may be addressed by LID, with no number to give).
    if coalesce((v_required->>'phone')::boolean, false) and coalesce(p_phone, '') = '' and coalesce(p_source, 'form') <> 'bot' then
